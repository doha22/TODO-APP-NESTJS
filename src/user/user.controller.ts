...
export class UserController {
    ...
    // @ApiOperation(User.modelName, 'Register')

   
    // async register(@Body() vm: User) {
    //   const created = await this._userService.register(vm);
    // if(created){return("sucessfully created")} 
    // else{
    // throw new HttpException('Some thing went wrong during creating user', HttpStatus.BAD_REQUEST)
    // }
    // }
    ...
    async register(@Body() vm: RegisterVm): Promise<UserVm> {
        ...
        if(!newUser){ throw new HttpException('Some thing went wrong during creating user', HttpStatus.BAD_REQUEST); }
        console.log("is user")
       this._userService.map(newUser, User, UserVm); 
       return newUser
